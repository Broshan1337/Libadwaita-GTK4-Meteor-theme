...
import me.pindour.adwaita.gui.screens.settings.AdwaitaEntityTypeListSettingScreen;
...
import me.pindour.adwaita.gui.themes.adwaita.widgets.pressable.WAdwaitaCheckbox;
...
import static me.pindour.adwaita.utils.WidgetUtils.reset;
import static meteordevelopment.meteorclient.MeteorClient.mc;

public class AdwaitaSettingsWidgetFactory extends SettingsWidgetFactory {
    private static final SettingColor WHITE = new SettingColor();
    private final AdwaitaGuiTheme theme;

    public AdwaitaSettingsWidgetFactory(AdwaitaGuiTheme theme) {
        super(theme);
    ...
    private void boolW(WTable table, BoolSetting setting) {
        ...
        WAdwaitaCheckbox checkbox = (WAdwaitaCheckbox) list.add(theme.checkbox(setting.get())).widget();
    ...
    private void intW(WTable table, IntSetting setting) {
        WAdwaitaIntEdit edit = table.add(theme.adwaitaIntEdit(setting)).expandX().widget();
    ...
    private void doubleW(WTable table, DoubleSetting setting) {
        WAdwaitaDoubleEdit edit = table.add(theme.adwaitaDoubleEdit(setting)).expandX().widget();
    ...
    private void genericW(WTable table, GenericSetting<?> setting) {
        ...
        WButton edit = list.add(theme.button(AdwaitaBuiltinIcons.EDIT.texture())).widget();
    ...
    private void colorW(WTable table, ColorSetting setting) {
        ...
        WColorPicker colorPicker = list.add(theme.colorPicker(setting.get(), AdwaitaBuiltinIcons.EDIT.texture())).widget();
    ...
    private void keybindW(WTable table, KeybindSetting setting) {
        ...
        WAdwaitaKeybind keybind = list.add(theme.adwaitaKeybind(setting.title, setting.get(), setting.getDefaultValue())).expandX().widget();
    ...
    private void entityTypeListW(WTable table, EntityTypeListSetting setting) {
        selectW(table, setting, () ->
                mc.setScreen(theme.adwaitaEntityTypeListScreen.get()
                        ? new AdwaitaEntityTypeListSettingScreen(theme, setting)
    ...
    private void blockDataW(WTable table, BlockDataSetting<?> setting) {
        ...
        WButton button = list.add(theme.button(AdwaitaBuiltinIcons.EDIT.texture())).widget();
    ...
    private void fontW(WTable table, FontFaceSetting setting) {
        ...
        WAdwaitaLabel label = (WAdwaitaLabel) theme.label(getFontLabel(setting, setting.get().info.family()));
    ...
    private void colorListWFill(WTable t, ColorListSetting setting) {
        ...
        for (SettingColor color : setting.get()) {
            ...
            WColorPicker colorPicker = list.add(theme.colorPicker(color, AdwaitaBuiltinIcons.EDIT.texture())).padHorizontal(theme.pad()).widget();
            colorPicker.action = () -> {
    ...
    private void vector3dW(WTable table, Vector3dSetting setting) {
        ...
        WAdwaitaDoubleEdit x = addVectorComponent(internal, "X", setting.get().x, val -> setting.get().x = val, setting);
        WAdwaitaDoubleEdit y = addVectorComponent(internal, "Y", setting.get().y, val -> setting.get().y = val, setting);
        WAdwaitaDoubleEdit z = addVectorComponent(internal, "Z", setting.get().z, val -> setting.get().z = val, setting);
    ...
    private WAdwaitaDoubleEdit addVectorComponent(WTable table, String label, double value, Consumer<Double> update, Vector3dSetting setting) {
        WAdwaitaDoubleEdit component = table.add(theme.adwaitaDoubleEdit(label, setting.description, value, setting.min, setting.max, setting.decimalPlaces, setting.sliderMin, setting.sliderMax, setting.noSlider)).expandX().widget();
    ...
    private static class WSelectedCountLabel extends WAdwaitaLabel {
