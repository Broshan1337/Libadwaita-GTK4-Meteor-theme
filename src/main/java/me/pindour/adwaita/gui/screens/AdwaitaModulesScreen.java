...
public class AdwaitaModulesScreen extends TabScreen {
    ...
    public AdwaitaModulesScreen(GuiTheme theme) {
    ...
    public void initWidgets() {
        ...
        if (theme.adwaitaSearchScreen.get())
        ...
        // Credit
        add(theme.label("Adwaita Theme by Pindour")).bottom().right();
    }
    ...
    public boolean keyPressed(KeyInput input) {
        ...
        if (!theme.adwaitaSearchScreen.get()) return false;
        ...
        if (control && keyCode == GLFW_KEY_F) {
            mc.setScreen(new AdwaitaSearchScreen(theme));
            return true;
        }
    ...
    protected WWindow createCategory(WContainer c, Category category, List<Module> moduleList) {
        WAdwaitaWindow w = (WAdwaitaWindow) theme.window(category.name);
        w.id = category.name;
        w.padding = theme.pad();
        w.spacing = 0;
    ...
    protected WWindow createSearch(WContainer c) {
        WAdwaitaWindow w = (WAdwaitaWindow) theme.window("Search");
        w.id = "search";
        ...
        if (theme.categoryIcons()) {
            w.beforeHeaderInit = wContainer -> wContainer.add(theme.texture(AdwaitaBuiltinIcons.SEARCH.texture(), size)).centerY().pad(4).padHorizontal(10);
        }
    ...
    protected Cell<WWindow> createFavorites(WContainer c) {
        ...
        WAdwaitaWindow w = (WAdwaitaWindow) theme.window("Favorites");
        w.id = "favorites";
        w.padding = 0;
        w.spacing = 0;
        ...
        if (theme.categoryIcons()) {
            w.beforeHeaderInit = wContainer -> wContainer.add(theme.texture(AdwaitaBuiltinIcons.BOOKMARK_YES.texture(), size)).centerY().pad(4).padHorizontal(10);
        }
    ...
    protected class WCategoryController extends WContainer {
        ...
        public void init() {
            ...
            AdwaitaGuiTheme adwaitaTheme = (AdwaitaGuiTheme) theme;

            if (!adwaitaTheme.adwaitaSearchScreen.get())
    ...
    private GuiTexture getIconForCategory(Category category) {
        GuiTexture icon = AdwaitaIcons.getCategoryIcon(category.name);
        return icon != null ? icon : AdwaitaBuiltinIcons.QUESTION_MARK.texture();
    }
